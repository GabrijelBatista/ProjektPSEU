...
<body style="background: url('images/download.jpg'); background-size: 100%; text-align:center;">
	<div class="wrapper" style="text-align:center; position:center;">
		<section class="panel panel-primary" style="background:transparent">
			<div class="panel-heading" style="font-size:30px; background-color:rgba(45, 37, 117, 0.452)">
				ZAVRŠNI RADOVI
			</div>
				...
				<table class="table table-bordered">
					...
					@foreach($downloads as $down)
						<tr>
							...
							<td><a href = "mailto: {{$down['email']}}">{{$down['email']}}</a></td>
                            <td>{{$down['tema']}}</td>
							<td>{{$down['mentor']}}</td>
							<td>{{$down['prijava']}}</td>
							<td>{{$down['obrana']}}</td>
							<td>
								<form method="POST" action="{{route('insertfile3.update', $down->id)}}?{{time()}}" class='form-horizontal'>
									@csrf
							...
							<td>
								<form id='deleteFormId2' action="{{route('viewfile.delete', $down->id)}}?{{time()}}">
								...
								<td>
									<form id='deleteFormId' action="{{route('viewfile.delete2', $down->id)}}?{{time()}}">
